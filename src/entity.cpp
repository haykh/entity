...
[[nodiscard]]
constexpr bool registryContainsSupportedCombo() {
  bool is_supported = false;

#define NTT_CHECK_REGISTRY_ENTRY(E, METRIC, DIM)                               \
  if (!is_supported && E<METRIC<DIM>>::S == S &&                               \
      METRIC<DIM>::MetricType == M && DIM == D)                                \
...
[[nodiscard]]
constexpr bool registryCoversMetric(std::integer_sequence<int, Ds...>) {
  bool covered = true;

  ((covered = covered &&
              registryContainsSupportedCombo<S, M, static_cast<Dimension>(Ds)>()),
   ...);
...
[[nodiscard]]
constexpr bool registryCoversEngine(std::integer_sequence<int, Ms...> metrics,
                                    std::integer_sequence<int, Ds...> dimensions) {
  bool covered = true;

  ((covered = covered && registryCoversMetric<S, static_cast<ntt::Metric::type>(Ms)>(
                           dimensions)),
   ...);
...
[[nodiscard]]
constexpr bool registryCoversPGen(std::integer_sequence<int, Es...> engines,
...
using ActivePGen = user::PGen<ntt::SimEngine::SRPIC, metric::Minkowski<Dim::_1D>>; // type only used for traits

static_assert(registryCoversPGen<ActivePGen>(ActivePGen::engines,
                                             ActivePGen::metrics,
                                             ActivePGen::dimensions),
              "Problem generator compatibility lists include combinations "
              "missing from the registry. Add the new engine/metric/dimension "
              "triplets to NTT_ENGINE_METRIC_DIMENSION_REGISTRY.");
